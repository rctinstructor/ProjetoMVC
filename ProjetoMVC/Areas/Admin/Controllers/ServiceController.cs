        ...
        public IActionResult Upsert()
        {
            if (ModelState.IsValid)
            {
                ...
                if (ServVM.Service.Id==0)
                {
                    ...
                    using (var filetreams = new FileStream(Path.Combine(uploads, fileName+extension), FileMode.Create))
                    {
                        files[0].CopyTo(filetreams);
                    }
                ...
                else
                {
                    ...
                    if (files.Count > 0)
                    {
                        ...
                        using (var filetreams = new FileStream(Path.Combine(uploads, fileName + extension_new), FileMode.Create))
                        {
                            files[0].CopyTo(filetreams);
                        }
